...
{
	int error;
	if(!pg)pg = (void *)UTOP; // see more details in kern/syscall.c:sys_ipc_try_send and sys_ipc_recv.
...
{
	if(!pg)pg = (void *)UTOP; // see more details in kern/syscall.c:sys_ipc_try_send
	int error;
