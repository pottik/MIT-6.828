...
mem_init_mp(void)
{
	...
	for(;CPU_i < NCPU; ++CPU_i){
		uintptr_t startVa = KSTACKTOP - CPU_i * (KSTKSIZE + KSTKGAP) - KSTKSIZE;
		boot_map_region(kern_pgdir, startVa, KSTKSIZE, PADDR(percpu_kstacks[CPU_i]), (PTE_W | PTE_P));
	}
}
...
page_init(void)
{
	...
	// step 1) Mark page 0 as in use.
	pages[0].pp_ref = 0;
	pages[0].pp_link = NULL;
	// step 2) Mark [PGSIZE, npages_basemem * PGSIZE) as free.
	for(i = 1; i <= npages_basemem; ++i){
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}
	// step 3) Not allocate IO hole [IOPHYSMEM, EXTPHYSMEM).
	for(; i < extendedFree; ++i){
		pages[i].pp_ref = 0;
		pages[i].pp_link = NULL;	
	}
	// step 4) Mark rest physical memory of extended memory.Before we call pageinit(),
	//         we just allocate memory for page directory and pages array in function
	//         mem_init() by using boot_alloc(),which means that the pages from nextfree
	//         to the end should be free!
	for(; i < npages; ++i){
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];	
	...
	*/
	// above all:
	// Tips:Notes boot_alloc return virtual address!
	size_t i, extendedFree = PGNUM(PADDR(boot_alloc(0)));
	for(i = 0; i < npages; ++i){
		pages[i].pp_ref = 0;
...
page_alloc(int alloc_flags)
{
	...
	// step 1) examine wheter page_free_list is NULL.If NULL,there is nothing to allocate.Just return NULL.
	if(!page_free_list)return NULL;
	// step 2) extract a free pageInfo node from page_free_list.
	struct PageInfo *freePageInfo = page_free_list;
	page_free_list = page_free_list->pp_link;
	freePageInfo->pp_link = NULL;
	
	// step 3) fills the returned physical page with specil character.
...
{
	/* Origin detailed version.
	uint32_t pdx = PDX(va), ptx = PTX(va); // Page directory index, page table index
	...
	if(pde & PTE_P){
		...
		// Then obtain it's page table entry.KADDR(PTE_ADDR(pde) is the virtual address of page table.
		pte = (pte_t *) KADDR(PTE_ADDR(pde)) + ptx;
		
	}else{
		// In conrast, page directory not present.
		if(create && (pageInfo = page_alloc(ALLOC_ZERO))){
			++pageInfo->pp_ref; // increment reference count directly.
			pgdir[pdx] = page2pa(pageInfo) | PTE_P | PTE_U | PTE_W; // Permissions.
			pte = (pte_t *) KADDR(PTE_ADDR(pgdir[pdx])) + ptx;
		}else return NULL;
	}
	return pte;
	*/
	...
	return (pte_t *)KADDR(PTE_ADDR(*pde)) + PTX(va); // semantically equivalent to &((pte_t *)KADDR(PTE_ADDR(pde)))[ptx]
}
...
{	
	/* Origin version.A really subtle bug!XD
	pte_t *pageTableEntry = pgdir_walk(pgdir, va, 1);
	if(!pageTableEntry)return -E_NO_MEM;
	if(*pageTableEntry & PTE_P){
		if(PTE_ADDR(*pageTableEntry) != page2pa(pp)){
	...
	*pageTableEntry = page2pa(pp) | perm | PTE_P;
	return 0;
	*/
	// Above all:In order to avoid unnecessary subtle bugs,with comments above this function,we use a more elegant way
	// to handle this.
	pte_t *pageTableEntry = pgdir_walk(pgdir, va, 1);
	if(!pageTableEntry)return -E_NO_MEM;
	if(*pageTableEntry & PTE_P){
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	if(!pageTableEntry)return NULL; // return NULL if there is no page mapped at va.
	if(pte_store){
		// if pte_store is nonzero, then we store in it the address of the pte for this page.
		*pte_store = pageTableEntry;	
	}
	// return the page mapped at virtual address 'va'.Notes that the pa2page only accept physical address.
...
page_remove(pde_t *pgdir, void *va)
{
	...
	if(pageInfo){
		// there is physical page at that address.
